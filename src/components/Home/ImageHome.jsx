...
import { Html, Environment, useGLTF, ContactShadows, OrbitControls,} from "@react-three/drei"; // Helpers úteis
import { useInView } from 'react-intersection-observer';
import styles from "./styles/ImageHome.module.css";
import ContentPc from "./ContentPc"; // Componente que vai dentro da tela do pc
...
function Model({ startY = 5, targetY = 1, resetAnimation }) {
 ...
 useEffect(() => {
  ...
  // animação de flutuar
  useFrame((state) => { // useFrame é uma função que roda a cada “frame” da animação, como um filme.
    ...
    if (!entered) {
      group.current.position.y = THREE.MathUtils.lerp(group.current.position.y, targetY, 0.02); //0.05 -> Velocidade da interpolação
